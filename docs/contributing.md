...
- `packages/cli`: CLI entrypoint, command layer, config, memory, optimizer, and playbook core
...
4. Register the command in [packages/cli/src/index.ts](../packages/cli/src/index.ts).
5. Add tests if the command introduces new core behavior.

## Adding A Playbook

1. Add a YAML file under `templates/playbooks-repo/playbooks/`.
2. Follow the schema documented in [playbook-schema.md](playbook-schema.md).
...
1. Add the tool description to [packages/mcp/src/server.ts](../packages/mcp/src/server.ts).
